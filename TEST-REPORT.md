...
| Production after (`263f325`) | **77** | 2.8 s | 4.3 s | 0 | 4.9 s | 152 KB preloaded / 9 files |

Desktop stays 98–99. **Why production does not follow the local gain:** `www.nobug.az` is proxied through Cloudflare (`server: cloudflare`), which adds a hop in front of Vercel's edge (observed ~250 ms server latency on every asset instead of ~5 ms locally), injects *Email Address Obfuscation* (`/cdn-cgi/l/email-protection` rewrites the `mailto:` links and adds `email-decode.min.js`) and the *Web Analytics* beacon (10 KB, high priority), and rewrites the ISR cache headers to `max-age=0, must-revalidate`. Lighthouse's simulation multiplies that per-request latency, hence FCP 2.8 s / LCP 4.3 s on production versus 0.9 s / 3.2 s for the identical build served directly. A real Chrome run against production with the same throttling reports FCP/LCP at **1.7 s** (`PerformanceObserver`). Recommended (account settings, not code): switch the `www` record to *DNS only* so Vercel serves the site directly (Vercel's guidance), or at least turn off *Email Address Obfuscation* and the automatic *Web Analytics* injection. The remaining simulated LCP gap (3.2 s vs the 2.5 s target) is Lighthouse's model tying the text LCP to the Inter Tight latin-ext file (90 KB — Azerbaijani letters live in that subset). In a real Chrome run with the same throttling, `PerformanceObserver` reports the hero paragraph as LCP at **1.1 s** (fallback font first, swap later). Next lever: self-hosted Inter Tight subset to the Azerbaijani/Turkish glyphs (~20 KB) via `next/font/local`.
